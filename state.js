...
// mặc định là vậy, nhưng khi người dùng yêu cầu rõ một phong cách nào đó, hoặc cốt truyện/thiết định
// thực sự đòi hỏi, thì lấy người dùng/thiết định làm chuẩn (đây là công cụ sáng tác, không cấm tiệt cốt truyện gay gắt).
// Bốn gạch đầu dòng tương ứng bốn kiểu cần tiết chế: cảm xúc ổn định ← cảm xúc cực đoan /
// lập trường công tâm ← thiên vị khi có xung đột / diễn đạt chừng mực ← lời lẽ sến sẩm nhờn nhợt /
// giọng điệu chân thành ← kiểu nói mỉa mai bóng gió.
...
export function buildCreativeChatSystemPrompt({ userName, charName, personaDesc = '', authorNote = '', outlineRaw = '', wiContext = '', recentCtx = '', almanacText = '' }) {
    ...
    // Khi đã có đại cương: sửa đại cương theo lối chỉnh sửa tăng dần, đừng viết lại toàn bộ
    // (nếu không, người dùng chỉ sửa một chi tiết mà cả 8 nút bị làm mới sạch, y như mở bản mới).
...
export function buildSpaceChatSystemPrompt({ userName, charName, personaDesc = '', authorNote = '', outlineRaw = '', wiContext = '', memText = '', recentCtx = '', pointList = '', lineList = '', almanacText = '' }) {
    const parts = [
        ...
        `Có ba hệ thống, hãy căn cứ vào từ ngữ người dùng dùng mà phân biệt nghiêm ngặt nên xuất loại thẻ nào:`,
        ...
        `- Nói "lịch / ngày tháng / lễ tết / ngày kỷ niệm / sinh nhật" (ghi lên lịch năm, mỗi năm lặp lại cố định) → xuất thẻ [Lịch], dùng <almanac_widget>`,
        ...
        `- month/day: số, month 1-12, day 1-31; năm không có ý nghĩa trong nhập vai, đừng ghi năm`,
